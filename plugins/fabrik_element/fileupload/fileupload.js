var FbFileUpload = new Class({
	...
	/**
	 * when in ajax form, on submit the list will call this, so we can remove
	 * the submit event if we dont do that, upon a second form submission the
	 * original submitEvent is used causing a js error as it still references
	 * the files uploaded in the first form
	 */
	...
	pluploadRemoveFile: function (e, file) {
		...
		Array.each(['data_', 'coords_', 'id_'], function (prefix) {

			var el = document.id(prefix + id);
			if (typeOf(el) !== 'null') {
				el.destroy();
			}
		});
	...
	onSubmit: function (form) {
		...
		if (typeOf(this.widget) !== 'null') {
			this.widget.images.each(function (image, key) {
				...
				// $$$ rob - seems reloading ajax fileupload element in ajax
				// form (e.g. from db join add record)
				// is producing odd effects where old fileupload object
				// constains info to previously uploaded image?
				if (typeOf(f) !== 'null') {
					delete image.img;
