FabRecordSet = new Class( {
	...
	doUpdate: function(json) {
		...
		this.form.formElements.each(function(oEl, key) {
			...
			try{
			if ($type(s) !== false) {
				this.view == 'form' ? oEl.update(s) : oEl.updateHTML(Encoder.htmlDecode(s));
			}else{
				this.view == 'form' ? oEl.update('') : oEl.updateHTML('');
			}
			}catch(err){
	...
	doNav:function(e, dir) {
		...
		switch(dir) {
			...
			if(this.options.ids.index == 0 || this.options.ids.index == 1) {
				...
				break;
			case 2:
				if(this.options.ids.index == this.options.ids.lastKey) {ok = false;}
	...
	watchButtons: function() {
		var n = this.form.getForm().getElement('ul.pagination');
		n.getElement('.paginateNext').addEvent('click', this.doNav.bindAsEventListener(this, [1]));
		n.getElement('.paginatePrevious').addEvent('click', this.doNav.bindAsEventListener(this, [-1]));
		n.getElement('.paginateLast').addEvent('click', this.doNav.bindAsEventListener(this, [2]));
		n.getElement('.paginateFirst').addEvent('click', this.doNav.bindAsEventListener(this, [0]));
	}
});