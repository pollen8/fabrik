...
class FabrikFEModelList extends JModelForm
{
	...
	*/
	...
	public function setLimits()
	{
		...
		{
			$limitLength = $input->get('limit', $params->get('rsslimit', 150));
	...
	public function getRecordCounts(&$element, $pks = array())
	{
		...
		$where = $listModel->_buildQueryWhere($input->get('incfilters', 0));
		if (!empty($pks))
		{
	...
	function _buildQuery()
	{
		...
		if ($this->mergeJoinedData())
		{
			...
			$query['where'] = $this->_buildQueryWhere($input->get('incfilters', 1));
	...
	protected function getJoinMergeTotalRecords()
	{
		...
		$totalSql .= " " . $this->_buildQueryWhere($app->input->get('incfilters', 1));
	...
	public function getGroupBy()
	{
		$table = $this->getTable();
		$app = JFactory::getApplication();
		$groupBy = $app->input->get('group_by', $table->group_by);
