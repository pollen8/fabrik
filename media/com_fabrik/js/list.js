...
/*global Fabrik:true, fconsole:true, Joomla:true, CloneObject:true, $A:true, $H:true,unescape:true,head:true,FbListActions:true,FbGroupedToggler:true,FbListKeys:true */
	...
	initialize: function (options) {
		...
		if (typeOf(c) !== 'null') {
			...
			c.addEvent('click', function (e) {
				...
				new Element('input', {'name': 'resetfilters', 'value': 1, 'type': 'hidden'}).inject(this.container);
		...
		if (del) {
			del.addEvent('click', function (e) {
				...
				if (r) {
					...
					if (typeOf(chx) !== 'null') {
						//if delete link is in hover box the we cant find the associated chx box
