	 		...
	 		new Element('table',  {width:'100%'}).adopt([
	 			new Element('tbody').adopt([
		       	...
		       	new Element('td', {width:'30%'}).adopt(
	   					new Element('input', {'class':'inputbox sub_values', 
	    				...
	    				new Element('td', {width:'30%'}).adopt(
	   					new Element('input', {'class':'inputbox sub_labels', type:'text', name:this.name+'[sub_labels][]', id:'sub_text_'+this.counter, size:20, value:sText})
	   				),
	   				new Element('td', {width:'10%'}).set('html',
	  	      		chx
	  	      		),
	   				new Element('td', {width:'20%'}).adopt(
	  	      		new Element('a', {'class':'removeButton',href:'#', id:'sub_delete_'+this.counter}).set('text', 'Delete')
	  	      	)
	 				])
	 			])
	 		])
