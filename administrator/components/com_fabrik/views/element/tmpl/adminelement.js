...
var fabrikAdminElement = new Class({
	...
	addJavascript: function(opt){
		...
		var content = new Element('table', {'class':'paramlist admintable adminform', 'id':'jsAction_'+this.jsCounter}).adopt(
			new Element('tbody', {'class':'adminform', 'id':'jsAction_'+this.jsCounter}).adopt([
				...
				new Element('tr').adopt(new Element('td', {'colspan':2,'class':'paramlist_key', 'styles':{'text-align':'left'}}).appendText(Joomla.JText._('COM_FABRIK_OR'))),
				new Element('tr').adopt(new Element('td', {'colspan':2}).adopt([
					evs, triggers, new Element('input', {'value':Joomla.JText._('COM_FABRIK_WHERE_THIS'), 'class':'readonly', 'disabled':'disabled', 'size':Joomla.JText._('COM_FABRIK_WHERE_THIS').length}), condition, new Element('input', {'name':'js_e_value[]','class':'inputbox', 'value':opt.params.js_e_value}) 
				])),
				new Element('tr').adopt(new Element('td', {'colspan':2}).adopt(new Element('a',{'href':'#','class':'removeButton','events':{
					'click':function(e){
						this.deleteJS(e);
						}.bind(this)
