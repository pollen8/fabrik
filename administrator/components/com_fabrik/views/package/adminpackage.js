...
AdminPackage = new Class({
	...
	makeBlockMenu : function() {
		var c = new Element('ul', {
			'id' : 'typeList'
		}).adopt([ new Element('li', {'class' : 'draggable typeList-list'}).adopt([
	...
	insertPage : function (page, id, label, type, dimensions){
		if(dimensions.width == 0){dimensions.width = 50;}
		if(dimensions.height == 0){dimensions.height = 50;}
		...
		var label = new Element('span', {'class':'handlelabel'}).set('text', label);
		...
		c.addEvent('mousedown', function(e){window.fireEvent('fabrik.page.add', [c]);});
		page.page.adopt(c);
	},
